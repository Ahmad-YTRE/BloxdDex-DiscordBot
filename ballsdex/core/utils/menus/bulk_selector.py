...
class BaseBulkSelector(Container):
    ...
    @selector_row.select(placeholder=f"Select {settings.plural_collectible_name}")
    ...
    async def on_confirm(self, interaction: Interaction, selected_ids: set[int]):
        """
        Called when the user confirms their selection. `selected_ids` is the set of BallInstance
        primary keys chosen across all pages.
        """
